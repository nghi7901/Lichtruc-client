...
const AdministrativeStatistics = () => {
    ...
    const handleExportExcel = async () => {
        try {
            const response = await fetch(`${process.env.REACT_APP_SERVER_URL}/statistics/export-excel`, {
                credentials: 'include',
                method: 'POST',
                headers: {
            ...
            link.href = url;
            link.setAttribute('download', 'ThongKeGiangVien.xlsx');
        ...
        <>
                ...
                {data?.totals ? (
                    <>
                        <div className="d-flex justify-content-between align-items-center row py-1 mx-4 gap-md-0">
                            <div>
                                <label>Số buổi trực được đăng ký:</label> &nbsp;
                                <Tag
                                    value={data.totals.totalRegistered}
                                    severity="success"
                                    style={{fontSize: '1.25rem'}}
                                />
                            </div>
                            <div>
                                <label>Số lượng yêu cầu được gửi:</label> &nbsp;
                                <Tag
                        ...
                        <div className="d-flex  justify-content-between align-items-center row py-1 mx-4 gap-md-0">
                            <div>
                                <label>Số buổi trực được thực hiện:</label> &nbsp;
                                <Tag
                                    value={data.totals.totalPresent}
                                    severity="success"
                                    style={{fontSize: '1.25rem'}}
                                />
                            </div>
                            <div>
                                <label>Số yêu cầu được duyệt:</label> &nbsp;
                                <Tag
                        ...
                        <div className="d-flex justify-content-between row py-1 mx-4 gap-md-0">
                            <div>
                                <label>Tổng số buổi vắng trực:</label> &nbsp;
                                <Tag
                                    value={data.totals.totalAbsent}
                                    severity="danger"
                                    style={{fontSize: '1.25rem'}}
                                />
                            </div>
                            <div>
                                <label>Số yêu cầu bị từ chối:</label> &nbsp;
                                <Tag
                                    value={data.totals.totalRejected}
                                    severity="danger"
                                    style={{fontSize: '1.25rem'}}
                                />
                            </div>
            ...
            <div className="card">
                ...
                <DataTable ref={dt} value={data?.lecturers}
                    ...
                    ></Column>
                    <Column field="totalRegistered" header="Số buổi đăng ký" sortable
                        style={{ minWidth: '5rem', fontSize: '0.9rem' }}
                    ></Column>
                    <Column field="totalPresent" header="Số buổi đã trực" sortable
                        style={{ minWidth: '5rem', fontSize: '0.9rem' }}
                    ></Column>
                    <Column field="totalAbsent" header="Số buổi vắng" sortable
                        style={{ minWidth: '5rem', fontSize: '0.9rem' }}
                    ></Column>
                    <Column field="totalRequests" header="Số yêu cầu" sortable
                        style={{ minWidth: '5rem', fontSize: '0.9rem' }}
                    ></Column>
                    <Column field="totalApproved" header="Yêu cầu được duyệt" sortable
                        style={{ minWidth: '5rem', fontSize: '0.9rem' }}
                    ></Column>
                    <Column field="totalRejected" header="Yêu cầu bị từ chối" sortable
                        style={{ minWidth: '5rem', fontSize: '0.9rem' }}
                    ></Column>
                </DataTable>
            </div>
...
export default AdministrativeStatistics;